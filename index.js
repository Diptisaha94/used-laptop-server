...
async function run() {
  ...
  app.delete('/products/:id', async (req, res) => {
    ...
    const result = await laptopProductsCollection.deleteOne(query);
    res.send(result);
});
...
app.get('/users/buyer/:email', async (req, res) => {
  ...
  res.send({ isBuyer: user?.optionRole === 'buyer' });
     ...
     app.get('/allproduts',async(req,res)=>{
      const query = {};
      const result=await laptopProductsCollection.find(query).limit(3).toArray();
      res.send(result);
   });
...
app.put('/myproducts/:email', async (req, res) => {
  const email = req.params.email;
  const filter = {email}
  const options = { upsert: true };
  const updatedDoc = {
